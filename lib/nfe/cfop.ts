/**
 * Tabela CFOP → descrição. Gerada a partir da planilha oficial do escritório;
 * as descrições são reproduzidas exatamente como vêm de lá, sem reescrita.
 *
 * Tabela estática de propósito: CFOP é uma lista fixa e igual pra todo mundo,
 * então não vale uma tabela no banco nem uma tela de importação — atualizar é
 * editar este arquivo e subir.
 *
 * NÃO EDITE À MÃO linha a linha: regenere a partir da planilha de origem.
 * 590 códigos. Exceção: o 5151 vem com a descrição vazia na
 * planilha e foi completado manualmente ("Transferência de produção do
 * estabelecimento") — se a planilha for corrigida, isso deixa de ser exceção.
 */
export const CFOP_DESCRICOES: Record<string, string> = {
  ...
  "1131": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  "1132": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  "1135": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  ...
  "1159": "Transferência p/ utilização na prestação de serviço",
  ...
  "2131": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  "2132": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  "2135": "Compra p/ utilização na prestação de serviço sujeita ao ISSQN",
  ...
  "2159": "Transferência p/ utilização na prestação de serviço",
  ...
  "5213": "Devolução de compra p/ utilização na prestação de serviço",
  "5214": "Devolução de compra p/ utilização na prestação de serviço",
  "5215": "Devolução de compra p/ utilização na prestação de serviço",
  ...
  "6213": "Devolução de compra p/ utilização na prestação de serviço",
  "6214": "Devolução de compra p/ utilização na prestação de serviço",
  "6215": "Devolução de compra p/ utilização na prestação de serviço",
  ...
  "7667": "Venda de combustível ou lubrificante a consumidor ou usuário final Classificam-se neste código as vendas de combustíveis ou lubrificantes a consumidor ou a usuário final, em embarcações ou aeronaves, nacionais ou estrangeiras, exclusivamente em tráfego",
...
/** Descrição do CFOP, ou null quando o código não está na tabela (ou vem vazio). */
export function cfopDescricao(cfop: string | null | undefined): string | null {
  const c = (cfop ?? "").replace(/\D/g, "");
  return CFOP_DESCRICOES[c] ?? null;
}

/** "5102 — Venda de produção do estabelecimento"; só o código quando não há descrição. */
export function cfopLabel(cfop: string | null | undefined): string {
  const c = (cfop ?? "").trim();
  if (!c) return "—";
  const desc = cfopDescricao(c);
  return desc ? `${c} — ${desc}` : c;
}

/** Recorte da tabela só com os CFOPs usados — evita mandar os 590 códigos pro client. */
export function cfopSubset(cfops: (string | null | undefined)[]): Record<string, string> {
  const out: Record<string, string> = {};
  for (const c of cfops) {
    const desc = cfopDescricao(c);
    if (desc) out[(c ?? "").trim()] = desc;
  }
  return out;
}
