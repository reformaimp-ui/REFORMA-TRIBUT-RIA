...
/**
 * Primeiro acesso sem link de e-mail: a pessoa confirma que é dona do e-mail
 * digitando um código de 6 dígitos (signInWithOtp + verifyOtp), evitando
 * depender da configuração de Redirect URL do Supabase (que varia por
 * ambiente e causou link quebrado apontando para localhost).
 */
export function PrimeiroAcessoForm() {
  ...
  const sendCode = async (e: React.FormEvent) => {
    ...
    setPending(false);
    if (error) return setError("Este e-mail não foi convidado por nenhum escritório. Confira se digitou certo.");
    setStep("otp");
  };
  ...
  return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }}>
      <div style={{ width: "100%", maxWidth: 360, display: "flex", flexDirection: "column", gap: 26 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-.01em" }}>Primeiro acesso</div>
          <div style={{ fontSize: 13, color: "#8a8d98", marginTop: 6 }}>
            {step === "email" ? "Informe o e-mail que te convidaram para o escritório." : null}
